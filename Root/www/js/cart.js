  ...
  app.service('eCart',['$http', function($http){
    ...
    this.addToCart = function(product){
      ...
      if (productInCart) {
        ...
        product.realQty = product.quantity;
	...
	//-----------------------
	 this.addInQty = function(product) {
		 this.isAvailable =true;
		this.cartProducts.forEach(function(prod, i, prods){
        if (product.model === prod.id) {
			if(prod.quantity >0){
	 ...
	 this.minusInQty = function(product) {
		 this.isAvailable =true;
		this.cartProducts.forEach(function(prod, i, prods){
        if (product.model === prod.id) {
		  ++prod.quantity;
		  --prod.purchaseQuantity;
		   if(prod.purchaseQuantity<=0)this.removeProduct(product);
		    this.updateTotal();
        }
      }.bind(this));
